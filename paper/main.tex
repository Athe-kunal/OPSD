...
\title{Positional Self-Distillation for Large Language Models}
...
We build on On-Policy Self-Distillation (OPSD)~\cite{opsd2025}, which trains a student model by minimizing a forward KL divergence against a fixed teacher, and introduce four position-selective variants that restrict distillation to all tokens within the first, middle, or last sentence of each reasoning paragraph, or to the first token of each paragraph.
...
Our preliminary experiments on Qwen3-1.7B across AIME24, AIME25, and HMMT25 show mixed but occasionally promising results: \texttt{parafirsttok\_topk1} reaches 40.8\% Avg@8 on AIME25 at step 100, compared to the full-token OPSD baseline of 37.9\% at that step.
...
Rather, we offer this as a useful datapoint: a sparser distillation signal concentrated at structurally motivated sentence positions can sometimes match or briefly exceed full-token distillation, which motivates further investigation.
\end{abstract}
...
Methods like GRPO~\cite{shao2024deepseekmath} and self-distillation~\cite{opsd2025,hubotter2026rl,shenfeld2026continual} have shown that a student model can be improved by learning from a stronger (or fixed) teacher's output distribution over long chain-of-thought completions.
...
Within a paragraph, we expect the first sentence to carry more uncertainty than the middle or the end, which tend to elaborate on an already-committed direction.
Paragraph boundaries themselves, however, turn out to be a more nuanced story: the very first token of a new paragraph (typically a newline or function word) is nearly deterministic, and our analysis reveals it is in fact the \emph{lowest}-entropy position (Section~\ref{sec:motivation}).
...
We then introduce four position-selective distillation variants on top of OPSD and evaluate them on three mathematical reasoning benchmarks (Section~\ref{sec:experiments}).
...
\begin{itemize}
    ...
    \item Four position-selective distillation variants that restrict training signal to specific sentence positions within paragraphs (Section~\ref{sec:method}).
...
Positional selection within generated reasoning chains — restricting distillation to structurally motivated token positions — has received less attention.
...
\midrule
Start  & $-0.188$ & $1.713$ & $[-0.193,\,-0.183]$ & 535{,}999 \\
Middle & $-0.083$ & $0.567$ & $[-0.085,\,-0.080]$ & 212{,}273 \\
End    & $-0.100$ & $0.553$ & $[-0.101,\,-0.098]$ & 497{,}750 \\
Single & $-0.118$ & $1.220$ & $[-0.119,\,-0.117]$ & 8{,}871{,}007 \\
\bottomrule
\end{tabular}
\end{table}
...
Start $>$ End    & $2.43 \times 10^{-59}$ & significant \\
Middle $>$ End   & $0.00$ & highly significant \\
\bottomrule
\end{tabular}
\end{table}

While the means are all negative (student slightly overconfident relative to teacher), the variance at the start position is much larger ($\text{std}=1.713$ vs.\ $0.567$ for middle and $0.553$ for end), meaning the signal is more extreme in both directions.
We hypothesize that these high-variance tokens represent decision points where the student's predictions diverge most from the teacher, making them more informative training targets.

\paragraph{A caveat on paragraph-first tokens.}
We also separately analyzed the first token of each paragraph (token-0 of the first sentence), since paragraph boundaries are natural structural anchors.
Counter-intuitively, these tokens are the \emph{lowest}-entropy position: mean KL proxy $= -0.967$ (std $= 5.002$, $n = 461{,}676$), versus $-0.078$ for the rest of first-sentence tokens.
A Mann-Whitney test confirms that token-0 is stochastically \emph{lower} than the remaining tokens ($p = 1.00$ for the ``tok-0 $>$ rest'' alternative).
This makes sense in retrospect: the first token of a new paragraph is typically a formatting or function word (``\textbackslash n'', ``The'', ``So'') whose identity is nearly deterministic given context, so the student is extremely confident and contributes almost no loss.
The entropy is concentrated in the body of the first sentence, not its opening token.
This foreshadows the underwhelming performance of \texttt{parafirsttok\_topk1} in our experiments (Section~\ref{sec:results}).
...
For each selected sentence position, all tokens within that sentence contribute to the distillation loss.
All tokens outside the selected position receive zero loss weight (implemented by setting their labels to $-100$).

We test the following four variants, plus the full-token OPSD baseline:
\begin{enumerate}
    \item \textbf{firstsent\_topk1}: all tokens in the first sentence of each paragraph
    \item \textbf{middlesent\_topk1}: all tokens in the middle sentence of each paragraph
    \item \textbf{lastsent\_topk1}: all tokens in the last sentence of each paragraph
    \item \textbf{parafirsttok\_topk1}: only the single first token of each paragraph
\end{enumerate}
...
\texttt{firstsent\_topk1} and \texttt{lastsent\_topk1} occasionally match or slightly exceed the baseline on AIME24 and AIME25 at certain checkpoints.
This is notable because these variants restrict distillation to a single sentence per paragraph, achieving similar performance with a substantially sparser training signal.
...
\textbf{Paragraph-first token is a low-entropy anchor.}
\texttt{parafirsttok\_topk1} shows variable behavior across benchmarks.
The entropy analysis in Section~\ref{sec:motivation} explains why: the first token of a paragraph (typically a newline, ``The'', or ``So'') has a mean KL proxy of $-0.967$, far more negative than any other group, meaning the student is already highly confident there.
Restricting distillation to these near-zero-loss tokens provides very little gradient signal, which likely explains the inconsistent and generally unimpressive results for this variant.
...
The sentence-level selection granularity is coarse — a natural next step is to select individual high-KL tokens within the chosen sentence rather than all tokens.
...
\bibitem{shenfeld2026continual}
Idan Shenfeld, Mehul Damani, Jonas H\"{u}botter, and Pulkit Agrawal.
\newblock Self-distillation enables continual learning.
\newblock {\em arXiv preprint arXiv:2601.19897}, 2026.

\bibitem{hubotter2026rl}
Jonas H\"{u}botter, Frederike L\"{u}beck, Lejs Behric, Anton Baumann, Marco Bagatella, Daniel Marta, et~al.
\newblock Reinforcement Learning via Self-Distillation.
\newblock {\em arXiv preprint arXiv:2601.20802}, 2026.
...
Shenzhi Wang, Le~Yu, Chang Gao, Chujie Zheng, Shixuan Liu, Rui~Lu, et~al.
\newblock Beyond the 80/20 rule: High-entropy minority tokens drive effective reinforcement learning for LLM reasoning.
\newblock In {\em Advances in Neural Information Processing Systems}, volume~38, pages 115452--115486, 2025.
